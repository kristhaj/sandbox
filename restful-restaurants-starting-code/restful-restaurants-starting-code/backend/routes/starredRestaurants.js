...
router.delete("/:id", (req, res) => {
  ...
  const newListOfStarredResaurants = STARRED_RESTAURANTS.filter(
    (starredRestaurant) => starredRestaurant.id != id
  );

  if(STARRED_RESTAURANTS.length === newListOfStarredResaurants.length) {
    res.sendStatus(404);
    return;
  }

  STARRED_RESTAURANTS = newListOfStarredResaurants;
  res.sendStatus(200);
});
